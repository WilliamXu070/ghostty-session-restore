...
class FlowRunner:
    ...
    def cmd_t(self, from_state: str) -> None:
        ...
        env = {
            "NEWMUX_NEW_TAB_OPEN_TAB": "0",
            "NEWMUX_SOCKET": self.flow.get("socket_name", "newmux-dev"),
            "NEWMUX_SOCKET_PATH": socket_path,
        }
        self.run_cmd(
            [str(ROOT / "scripts" / "request-newmux-new-tab.sh"), socket_path, pane_id],
            env_extra=env,
        )
        self.run_cmd(
            [str(ROOT / "scripts" / "start-newmux-fresh.sh")],
            env_extra={**env, "NEWMUX_STARTER_PRINT_WINDOW": "1"},
        )
    ...
    def cmd_w(self, from_state: str, target: str = "primary_active") -> None:
        ...
        self.event("action.end", action="cmd_w", socket_path=self.socket_path(), target_pane=pane_id)
